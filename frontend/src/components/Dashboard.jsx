...
const Dashboard = ({ onLogout, API_URL, username }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_URL}/api/store/check`, {
        redirectUrl,
      });
      ...
      const mappedMessage =
        serverMessage ===
        "Please provide a Riot redirect URL containing an access_token."
  ...
  const renderStorefront = () => {
        ...
        <div className="p-3 rounded-2xl border bg-valorant-dark border-white/5 text-center flex flex-col justify-between min-h-[16rem]">
          <div className="relative h-40 w-full flex items-center justify-center p-2 mb-3 rounded-2xl bg-black/10 overflow-hidden">
            ...
            {priceInfo.discountPercent ? (
              <span className="absolute top-2 left-2 rounded-full bg-valorant-red text-[10px] font-semibold uppercase tracking-[0.12em] text-white px-2 py-0.5 shadow-sm">
                -{priceInfo.discountPercent}%
              </span>
            ) : null}
            {meta.contentTier?.displayIcon ? (
              <div className="absolute bottom-2 left-2 w-8 h-8 rounded-full bg-white/10 ring-1 ring-white/20 shadow-[0_0_8px_rgba(0,0,0,0.25)] p-[2px] backdrop-blur-sm">
                <img
                  src={meta.contentTier.displayIcon}
                  alt={meta.contentTier.displayName || "Tier"}
          ...
          <div className="text-sm font-bold text-white tracking-wide whitespace-normal break-words min-h-[4rem] leading-snug">
            <span className="block">
              {meta.displayName || label || "Unknown"}
            </span>
          </div>
          {priceInfo.discountPercent ? (
            <div className="mt-2 flex flex-col items-center gap-1">
              {priceInfo.discountedPrice ? (
                <span className="text-sm font-bold text-white">
                  {priceInfo.discountedPrice} VP
                </span>
              ) : null}
              {priceInfo.basePrice ? (
                <span className="text-[11px] text-valorant-gray line-through">
                  {priceInfo.basePrice} VP
                </span>
              ) : null}
            </div>
          ) : priceText ? (
            <div className="text-xs text-valorant-gray mt-2">{priceText}</div>
          ) : null}
        </div>
      );
    };
    ...
    const formatBundleItemPrice = (item) => {
      ...
      if (discounted === 0) {
        return {
          primary: t.featuredBundleFree,
          secondary: base ? `${base} VP` : null,
          discountPercent,
        };
      }
      ...
      sections.push(
        <div key="featuredBundles" className="mt-3 space-y-3">
          ...
          {featuredBundles.map((bundle, bundleIdx) => {
            ...
            const bundleKey =
              bundle.bundleId || bundle.dataAssetId || `bundle-${bundleIdx}`;
            const isOpen = !!expandedBundles[bundleKey];
            const bundleName =
              bundle.bundleMeta?.displayName || t.featuredBundleTitle;
            const bundleImage =
              bundle.bundleMeta?.verticalPromoImage ||
              bundle.bundleMeta?.displayIcon ||
              null;
            ...
            const totalBase =
              bundle.totalBaseCost?.["85ad13f7-3d1b-5128-9eb2-7cd8ee0b5741"];
            const bundleDiscountPercent = formatDiscountPercent(
              bundle.totalDiscountPercent,
            );
            const itemCountLabel = (
              t.featuredBundleItemCount || "{count} items"
            ).replace("{count}", bundle.items.length);

            return (
              <div
                key={bundleKey}
                className="rounded-xl border border-white/5 bg-valorant-darker/60 overflow-hidden"
              >
                <button
                  type="button"
                  onClick={() =>
                ...
                >
                  {bundleImage ? (
                    <img
                      src={bundleImage}
                      alt={bundleName}
                      className="h-28 w-28 object-contain rounded-lg bg-black/20 shrink-0"
                    />
                  ) : (
                    <div className="h-28 w-28 rounded-lg bg-black/20 shrink-0 flex items-center justify-center text-[10px] text-valorant-gray uppercase">
                      Bundle
                    </div>
                  )}

                  <div className="flex-1 min-w-0 pl-1">
                    <h6 className="text-base font-bold text-white truncate">
                      {bundleName}
                    </h6>
                    <p className="mt-1 text-xs text-valorant-gray">
                      {itemCountLabel}
                    </p>
                    {totalDiscounted != null ? (
                      <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
                        {totalBase != null && totalBase !== totalDiscounted ? (
                          <span className="text-valorant-gray line-through">
                            {totalBase} VP
                          </span>
                        ) : null}
                        <span className="text-white font-semibold">
                          {totalDiscounted === 0
                            ? t.featuredBundleFree
                            : `${totalDiscounted} VP`}
                        </span>
                        {bundleDiscountPercent ? (
                          <span className="rounded-full bg-valorant-red/90 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">
                            -{bundleDiscountPercent}%
                          </span>
                        ) : null}
                      </div>
                    ) : null}
                    <p className="mt-1 text-[10px] text-valorant-gray">
                      {isOpen
                        ? t.featuredBundleCollapse
                        : t.featuredBundleExpand}
                    </p>
                  </div>

                  <ChevronDown
                    className={`w-5 h-5 text-valorant-gray shrink-0 transition-transform ${isOpen ? "rotate-180" : ""}`}
                  />
                </button>
                ...
                {isOpen ? (
                  <div className="border-t border-white/5 p-3 space-y-2 bg-black/10">
                    {bundle.items.map((item, idx) => {
                      ...
                      return (
                        <div
                          key={`${bundleKey}-item-${idx}`}
                          className="flex items-center gap-3 rounded-lg border border-white/5 bg-valorant-dark/80 p-3"
                        >
                          ...
                          <div className="text-right shrink-0">
                            {priceInfo.discountPercent ? (
                              <span className="mb-1 block rounded-full bg-valorant-red/90 px-2 py-0.5 text-[10px] font-semibold text-white">
                                -{priceInfo.discountPercent}%
                              </span>
                            ) : null}
                            {priceInfo.primary ? (
                            ...
                            {priceInfo.secondary ? (
                              <div className="text-[11px] text-valorant-gray line-through">
                                {priceInfo.secondary}
                              </div>
                            ) : null}
                          </div>
    ...
    if (storefront.skinsPanel?.offers?.length) {
      sections.push(
        <div
          key="skinsPanel"
          className="mt-3 p-4 bg-valorant-darker/60 rounded-lg border border-white/5"
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {storefront.skinsPanel.offers.map((offer, idx) => (
              <div key={idx} className="min-h-0">
                {renderCard(
                  offer,
                  offer.metadata?.displayName,
                  offer.priceVP ? `${offer.priceVP} VP` : "",
                )}
              </div>
            ))}
          </div>
      ...
      <header className="glass-panel border-b border-white/5 px-6 py-4 shrink-0 relative z-20">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <h1 className="text-md font-bold tracking-widest uppercase text-valorant-red">
                {t.brand || "VALOCHECK"}
              </h1>
              <p className="text-[10px] text-valorant-gray font-mono mt-0.5 flex items-center gap-1.5">
                <span className="relative flex h-2 w-2">
                  <span
                    className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${serverHealth.ok ? "bg-emerald-400" : "bg-red-400"}`}
                  ></span>
                  <span
                    className={`relative inline-flex rounded-full h-2 w-2 ${serverHealth.ok ? "bg-emerald-500" : "bg-red-500"}`}
                  ></span>
                </span>
          ...
          <div className="flex items-center gap-3 ml-auto">
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`px-2 py-1 rounded ${language === "vn" ? "bg-valorant-red text-white" : "text-valorant-gray hover:text-white"}`}
              >
                VN
              </button>
            </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 md:p-6 flex flex-col gap-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start w-full">
            ...
            <div className="glass-panel rounded-xl p-5 border border-white/5 flex flex-col justify-between">
              <div>
                <h2 className="text-sm font-bold uppercase tracking-wider text-valorant-gold flex items-center gap-2">
                  <Activity className="w-4 h-4 text-valorant-red" />{" "}
                  {t.quickStepsTitle}
                </h2>
                <ol className="mt-4 space-y-3 text-sm text-valorant-gray">
                  <li>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-valorant-gold underline inline-flex items-center gap-1"
                    >
                      {language === "vn" ? "trang này" : "login page"}{" "}
                ...
                <form onSubmit={handleSubmit} className="mt-4 space-y-3">
                  <textarea
                    ...
                    rows="5"
                    placeholder={t.pasteRedirectUrl}
                    className="w-full bg-valorant-darker border border-white/10 rounded-lg p-3 text-white placeholder-valorant-gray/40 focus:outline-none focus:border-valorant-red"
                  />
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-valorant-red hover:bg-valorant-red-hover text-white font-bold py-3 rounded-lg shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                  >
                    {loading ? (
                      <>
                ...
                {error && (
                  <div className="mt-3 text-sm text-valorant-red">{error}</div>
                )}
                {riotId && (
                  <div className="mt-3 text-base font-semibold text-emerald-400">
                    {t.riotIDLabel} {riotId} •{" "}
                    {language === "vn" ? "Vùng" : "Region"}: {shard || "ap"}
                  </div>
                )}
              </div>

              {(storefront || profile) && riotId && (
                <div className="mt-4 text-sm text-valorant-gray">
                  <span>{t.checkAnotherAccountPart1} </span>
                  <a
                    href={t.signOutUrl}
                    target="_blank"
                    rel="noreferrer"
                    className="text-valorant-gold underline hover:text-white"
                  >
                    {t.checkAnotherAccountLink}
                  </a>
          ...
          <section className="lg:col-span-7 flex flex-col">
            <div className="glass-panel rounded-xl p-5 border border-white/5 flex-1 flex flex-col">
              {!storefront && !profile ? (
                <div className="flex-1 flex flex-col items-center justify-center text-center text-valorant-gray p-6">
                  <Sparkles className="w-12 h-12 mb-3 text-valorant-red/50" />
                  <p className="text-base font-medium">{t.waitingPasteUrl}</p>
                </div>
              ) : (
                <div className="flex-1 flex flex-col">
                  ...
                  {activeTab === "store" ? (
                    storefront ? (
                      ...
                      <div className="text-sm text-valorant-gray py-10 text-center">
                        {t.storeUnavailable}
                      </div>
                    )
                  ) : (
        ...
        <div className="w-full flex justify-center items-center">
          <div className="w-full max-w-2xl rounded-lg overflow-hidden border border-white/10 shadow-lg">
            <a href="/demo.gif" target="_blank" rel="noopener noreferrer">
              <img
                src="/demo.gif"
                alt="DEMO: How to check your Valorant store and inventory"
                className="w-full h-auto object-cover block mx-auto cursor-pointer"
              />
            </a>
          </div>
        </div>
